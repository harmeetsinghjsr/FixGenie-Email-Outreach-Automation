...
from fixgenie.common.models import EmailStatus, Lead, OutreachStatus, clean_business_name
...
class OutreachEngine:
    ...
    def _context(self, lead: Lead) -> dict:
        ...
        # Clean the (often scraped) business name at render time, so even leads
        # already stored with messy names get a tidy name in the email.
        name = clean_business_name(lead.business_name) or lead.business_name
        return {
            "business_name": name,
            ...
            "city": city,
            # Per-lead personalization openers (may be empty). Each template
            # uses the one for its step: detail=step1, detail2=step2, detail3=step3.
            "detail": (lead.pitch_hook or "").strip(),
            "detail2": (lead.pitch_hook2 or "").strip(),
            "detail3": (lead.pitch_hook3 or "").strip(),
