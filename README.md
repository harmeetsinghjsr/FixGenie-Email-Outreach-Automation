...
Want the whole thing to run by itself every day — find leads, personalize them
(with AI-written follow-ups), push to the Sheet, and send outreach + follow-ups
— even when your computer is off? That's what
[.github/workflows/pipeline.yml](.github/workflows/pipeline.yml) does.

**It runs once a day** (default 13:00 UTC, weekdays — change the `cron` line to
your time) and executes:

```
bulk_find → personalize → enrich → validate → push → outreach
```

Discovery/personalize are best-effort, so a flaky search never blocks the send.
The daily cap (`DAILY_SEND_LIMIT`, set to 30 in the workflow) keeps volume safe.
...
# AUTOMATE the daily send (Windows, weekday mornings) - run once:
schtasks /Create /TN "FixGenie Outreach" /TR "\"%CD%\scripts\run_outreach.bat\"" /SC WEEKLY /D MON,TUE,WED,THU,FRI /ST 08:52
```
...
scripts/run_outreach.bat ← daily send runner for Windows Task Scheduler
